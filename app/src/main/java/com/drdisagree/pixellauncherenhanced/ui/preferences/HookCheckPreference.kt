...
class HookCheckPreference(
    ...
    private val intentFilterHooked =
        IntentFilter().apply {
    ...
    private fun updateUI() {
        holderRef?.get()?.itemView?.let { itemView ->
            if (hasBootlooped) {
                ...
                itemView
                    .findViewById<TextView>(R.id.summary)
    ...
    private val hookCheckSender: Runnable =
        object : Runnable {
            override fun run() {
                if (!isHookSuccessful) {
                    try {
    ...
    private val hookCheckReceiver: BroadcastReceiver =
